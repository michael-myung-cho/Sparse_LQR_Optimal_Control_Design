...
function [Fopt,Jopt, NTstep, numCG] = SH2_newtonCG(A,B1,B2,Q,R,IS,F0,tol)
...
% numCG: number of conjugate gradient steps
    ...
    for NTstep = 1 : NT_Max_Iter
        ...
        if ngradF/sqrt(size(gradF,1)*size(gradF,2)) < tol
            break;
        end

        % compute the Newton direction using the conjugate gradient scheme
        [Fnt, CGstep ]= newtonCG(A,B2,F,P,L,R,gradF,IS);
        
        
        numCG = numCG+CGstep+1;
