...
function [Fopt,Jopt, NTstep, numCD] = SH2_newtonCD_slow(A,B1,B2,Q,R,IS,F0,tol)
...
for NTstep = 1 : NT_Max_Iter
    ...
    [Fnt, Fnt_alt, CDstep ]= newtonCD(A,B2,F,P,L,R,IS, gradF);
    ...
    while 1
        ...
        if Jtemp > J
            fprintf('not a descend direction\n');
            stop = 1;
            break
        end
    ...
    % line search for alternate direction
