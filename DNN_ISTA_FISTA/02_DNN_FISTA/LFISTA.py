#!/usr/bin/python
"""
Building Learning Fast ISTA (DNN-FISTA) network for sparse LQR optimal control design
...
def build_LFISTA(prob,T,initial_gamma,initial_rho,untied=False):
    """
    ...
    for t in range(0,T):
        ...
        with tf.name_scope("layer"+str(t)):
            ...
            beta_ = tf.Variable(1.0, name='beta_{0}'.format(t), dtype=tf.float32)

            Fhat_ = beta_* Fmin_stateCheck(A_,B1_,B2_,R_,Q_, initial_rho, initial_gamma, Fhatpre_, GradJ_,rhoStep_,rhoThre_) + (1-beta_)*Fhatpre_

            layers.append(('LFISTA T='+str(t), Fhat_))
