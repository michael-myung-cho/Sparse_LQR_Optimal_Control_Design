%% Spares LQR Optimal Control
% Iterative Sparse Projection Algorithm (ISPA) vs. GraSP
...
%%%%%% ISPA  %%%%%%
fprintf('ISPA \n');
options_ISPA = struct('method','l0','gamval', [140,135,130,125,120,115,110,105,100,95,90,85,80,75,70,65,60],'rho',1,'maxiter',10000,'blksize',[1 3],'reweightedIter',1);
tISPAs=tic;
solpath_ISPA = ISPA_SparseK(A,B1,B2,Q,R,options_ISPA); % solpath_ista.F: for obtaining s_gamma
tISPAe=toc(tISPAs);
...
%%%%%% GraSP  %%%%%%
fprintf('GraSP \n');
s_gamma=[140,135,130,125,120,115,110,105,100,95,90,85,80,75,70,65,60];
tGraSPs=tic;
solpath_GraSP = GraSP_SparseK(A,B1,B2,Q,R,s_gamma); 
tGraSPe=toc(tGraSPs);
...
figure(1)
semilogx(options_ISPA.gamval,solpath_ISPA.nnz,'b-o','MarkerSize',15,'LineWidth',3)
...
hold on
semilogx(s_gamma,solpath_GraSP.cardn,'r-x','MarkerSize',15,'LineWidth',3)
h = get(gcf,'CurrentAxes');
...
%% complexity
figure(2)
bar([solpath_GraSP.tBuf;solpath_ISPA.tBuf]')
 
%% J value
figure(3)
bar([solpath_GraSP.Jopt;solpath_ISPA.J]')
